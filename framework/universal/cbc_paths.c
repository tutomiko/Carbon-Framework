...
#pragma endregion
	...
	for (uint_t i = 0; i < n_file_name_length; i++) {
		...
		do {
			chr = (char_t)prandom_rand(&o_random);
			...
			if (i > 0) 
				seed += s_file_name[i - 1];
			else {
				seed += n_file_name_length;
			}

			seed += rand();
		} while (!char_isalphabet(chr) && !(char_isnumber(chr) && (i > 0 || NULL != s_file_prefix)));

		prandom_seed(&o_random, seed);
...
#pragma endregion
	...
	for (uint_t i = 0; i < n_file_name_length; i++) {
		...
		do {
			chr = (wchar_t)prandom_rand(&o_random);
...
#pragma endregion
	...
	for (int_t i = 0; i < _MAX_DRIVE; i++) {
		...
		if (c == ':') {
			if ((i >= 1) && char_isalphabet(s_path[i - 1])) {
				b_is_absolute = true;
			}
...
#pragma endregion
	...
	for (int_t i = 0; i < _MAX_DRIVE; i++) {
		...
		if (c == L':') {
			if ((i >= 1) && wchar_isalphabet(s_path[i - 1])) {
				b_is_absolute = true;
			}
...
bool_t path_isrelative(__in string_t const s_path) {
	return !path_isabsolute(s_path);
}


CBCEXPORT
bool_t path_isrelative_w(__in wstring_t const s_path) {
	return !path_isabsolute_w(s_path);
}
...
CBCEXPORT
string_t path_resolve(__in string_t const s_path, __in_opt uint_t n_path_length, __in_opt string_t const s_context_path, __in_opt uint_t n_context_path_length, __inout string_t s_absolute_path, __out_opt uint_t* _n_absolute_path_length) {
#pragma region <locals>
	uint_t n_absolute_path_length = 0;
	uint_t n_cursor;
...
#pragma endregion
	...
	if ((NULL != s_context_path) && (n_context_path_length == 0)) {
...
CBCEXPORT
wstring_t path_resolve_w(__in wstring_t const s_path, __in_opt uint_t n_path_length, __in_opt wstring_t const s_context_path, __in_opt uint_t n_context_path_length, __inout wstring_t s_absolute_path, __out_opt uint_t* _n_absolute_path_length) {
#pragma region <locals>
	uint_t n_absolute_path_length = 0;
	uint_t n_cursor;
	uint_t n_last_delimiter;
