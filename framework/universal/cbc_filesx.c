...
#
#
FWEXPORT __success(return) bool_t file_destroy(__in string_t const file_path) {
	return file_zero(file_path) && file_delete(file_path);
}


FWEXPORT __success(return) bool_t file_destroy_w(__in wstring_t const file_path) {
	return file_zero_w(file_path) && file_delete_w(file_path);
}


FWEXPORT __success(return) bool_t file_moveto(__in string_t const s_src_file_path, __in string_t const s_directory_path) {
...
FWEXPORT __success(return) bool_t file_moveto_w(__in wstring_t const s_src_file_path, __in wstring_t const s_directory_path) {
#pragma region <locals>
	size_t  n_dst_file_path_length;
